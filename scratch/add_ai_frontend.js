...
// 1. Update layout.tsx
const layoutPath = 'g:\\AFYA LINKS\\apps\\web\\src\\app\\admin\\layout.tsx';
let layout = fs.readFileSync(layoutPath, 'utf8');

if (!layout.includes('AI & Campaigns')) {
    layout = layout.replace(
        "import { \n  LayoutDashboard,", 
        "import { \n  LayoutDashboard,\n  Bot,\n  Megaphone,"
    );
    // If it doesn't match multiline exactly, just add it to lucide-react imports:
    if (!layout.includes('Bot,')) {
        layout = layout.replace('LayoutDashboard,', 'LayoutDashboard,\n  Bot,\n  Megaphone,');
    }
...
// 2. Create ai-centre page
const aiCentreDir = 'g:\\AFYA LINKS\\apps\\web\\src\\app\\admin\\ai-centre';
if (!fs.existsSync(aiCentreDir)) {
    fs.mkdirSync(aiCentreDir, { recursive: true });
}
...
fs.writeFileSync(path.join(aiCentreDir, 'page.tsx'), pageContent);
